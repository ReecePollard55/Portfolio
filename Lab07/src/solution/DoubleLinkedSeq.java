...
public class DoubleLinkedSeq implements Cloneable
{
    ...
    {
        // code for addBefore
        DoubleNode de = new DoubleNode(element);
        if (manyNodes == 0)
        {           
            head = de;
            tail = de;
    ...
    public void addAll(DoubleLinkedSeq other) throws NullPointerException
    {
        ...
        DoubleLinkedSeq t = new DoubleLinkedSeq();
        for (DoubleNode cursor = other.head; cursor != null; cursor = cursor.getLink())
        {
            t.addAfter(cursor.getData());
        }
        for (DoubleNode cursor = t.head; cursor != null; cursor = cursor.getLink())
        {
            addAfter(cursor.getData());
        }
        cursor = x;
    }
 ...
 */
            ...
            DoubleLinkedSeq s2) throws NullPointerException
    {
        ...
        for (DoubleNode cursor = s1.head; cursor != null; cursor = cursor.getLink())
        {
            result.addAfter(cursor.getData());
        }
        for (DoubleNode cursor = s2.head; cursor != null; cursor = cursor.getLink())
        {
            result.addAfter(cursor.getData());
        }
        result.advance();
    ...
    public boolean equals(Object other)
    {
        ...
        if (this.manyNodes == ((DoubleLinkedSeq) other).size())
        {
            ...
            {
                if (cursor.getData() != ((DoubleLinkedSeq) other).getCurrent())
                {
                    return false;
                }
